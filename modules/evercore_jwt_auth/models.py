...
class TokenUser:
    ...
    def id(self) -> Union[int, str]:
        return self.token[jwt_auth_settings.USER_ID_CLAIM]
    ...
    def __getattr__(self, attr: str) -> Optional[Any]:
        ...
        token = self.token
        try:
            # try to access token attribute first
            return getattr(token, attr)
        except AttributeError:
            # get token item as fallback
            try:
                return self.token[attr]
            except KeyError:
                raise AttributeError(
                    f"{self.__class__} object has not attribute '{attr}'"
                )
